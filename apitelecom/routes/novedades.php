...
require_once __DIR__ . '/../controllers/NovedadController.php';
...
$controller = new NovedadController();
...
if (count($segments) >= 2 && $segments[0] === 'api' && $segments[1] === 'novedades') {
    $id = isset($segments[2]) && is_numeric($segments[2]) ? (int) $segments[2] : null;
    $action = $segments[3] ?? null;

    switch ($method) {
        case 'GET':
            if ($action === 'publicadas') {
                $controller->publicadas();
                break;
            }
            if ($id === null) {
                $controller->index();
